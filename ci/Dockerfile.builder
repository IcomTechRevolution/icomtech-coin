...
# icomtech_hash
RUN git clone https://github.com/icomtechpay/icomtech_hash
RUN cd icomtech_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} icomtech
RUN useradd -u ${USER_ID} -g icomtech -s /bin/bash -m -d /icomtech icomtech
...
RUN mkdir /icomtech-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /icomtech-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /icomtech-src
