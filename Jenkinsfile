...
// which are part of the IcomTech repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="icomtech-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /icomtech-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "icomtechpay-icomtech/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from icomtechpay-icomtech/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'icomtechpay-icomtech/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from icomtechpay-icomtech/develop"
            }
          }

          if (hasCache) {
            sh "cd /icomtech-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /icomtech-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /icomtech-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /icomtech-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /icomtech-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /icomtech-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /icomtech-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
