...
pkgdatadir = $(datadir)/icomtechcore
pkgincludedir = $(includedir)/icomtechcore
pkglibdir = $(libdir)/icomtechcore
pkglibexecdir = $(libexecdir)/icomtechcore
...
CONFIG_HEADER = $(top_builddir)/src/config/icomtech-config.h
CONFIG_CLEAN_FILES = libicomtechconsensus.pc share/setup.nsi \
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	$(srcdir)/libicomtechconsensus.pc.in \
	...
	$(top_srcdir)/src/config/icomtech-config.h.in \
...
ACLOCAL = ${SHELL} /tmp/IcomTech/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /tmp/IcomTech/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /tmp/IcomTech/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /tmp/IcomTech/build-aux/missing automake-1.15
AWK = gawk
BDB_CFLAGS = 
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = icomtech-cli
BITCOIN_DAEMON_NAME = icomtechd
BITCOIN_GUI_NAME = icomtech-qt
BITCOIN_TX_NAME = icomtech-tx
...
COPYRIGHT_HOLDERS_FINAL = The IcomTech Core developers
COPYRIGHT_HOLDERS_SUBSTITUTION = IcomTech Core
COPYRIGHT_YEAR = 2019
CPP = gcc -E
...
MAINT = 
MAKEINFO = ${SHELL} /tmp/IcomTech/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL = 
OTOOL64 = 
PACKAGE = icomtechcore
PACKAGE_BUGREPORT = https://github.com/icomtechpay/icomtech/issues
PACKAGE_NAME = IcomTech Core
PACKAGE_STRING = IcomTech Core 1.0.0
PACKAGE_TARNAME = icomtechcore
PACKAGE_URL = https://icomtech.org/
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /tmp/IcomTech
abs_srcdir = /tmp/IcomTech
abs_top_builddir = /tmp/IcomTech
abs_top_srcdir = /tmp/IcomTech
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /tmp/IcomTech/build-aux/install-sh
...
pkgconfig_DATA = libicomtechconsensus.pc
...
DIST_CONTRIB = $(top_srcdir)/contrib/icomtech-cli.bash-completion \
	       $(top_srcdir)/contrib/icomtech-tx.bash-completion \
	       $(top_srcdir)/contrib/icomtechd.bash-completion \
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info \
  leveldb_baseline.info test_icomtech_filtered.info total_coverage.info \
  baseline_filtered.info rpc_test.info rpc_test_filtered.info \
  leveldb_baseline_filtered.info test_icomtech_coverage.info test_icomtech.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/IcomTech-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/icomtech-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/icomtech-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/icomtech-config.h
$(top_srcdir)/src/config/icomtech-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/icomtech-config.h src/config/stamp-h1
libicomtechconsensus.pc: $(top_builddir)/config.status $(srcdir)/libicomtechconsensus.pc.in
...
$(OSX_APP)/Contents/MacOS/IcomTech-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/IcomTech-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/IcomTech-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_icomtech.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_icomtech -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_icomtech_filtered.info: test_icomtech.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#rpc_test.info: test_icomtech_filtered.info
...
#test_icomtech_coverage.info: baseline_filtered_combined.info test_icomtech_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_icomtech_filtered.info -o $@

#total_coverage.info: baseline_filtered_combined.info test_icomtech_filtered.info rpc_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_icomtech_filtered.info -a rpc_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_icomtech.coverage/.dirstamp:  test_icomtech_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_icomtech.coverage/.dirstamp total.coverage/.dirstamp

.INTERMEDIATE: $(COVERAGE_INFO)

clean-local:
	rm -rf coverage_percent.txt test_icomtech.coverage/ total.coverage/ qa/tmp/ cache/ $(OSX_APP)
