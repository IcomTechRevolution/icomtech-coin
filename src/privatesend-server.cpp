...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (fLiteMode) return; // ignore all IcomTech related functionality
...
/*
    ...
    allow endless transaction that would bloat IcomTech and make it unusable. To
...
{
    if (fLiteMode) return;        // disable all IcomTech specific functionality
