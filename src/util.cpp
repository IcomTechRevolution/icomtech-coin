...
const char * const BITCOIN_CONF_FILENAME = "icomtech.conf";
const char * const BITCOIN_PID_FILENAME = "icomtechd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "icomtech" is a composite category enabling all IcomTech-related debug output
                if(ptrCategory->count(std::string("icomtech"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\IcomTech
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\IcomTech
    // Mac: ~/Library/Application Support/IcomTech
    // Unix: ~/.icomtech
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "IcomTech";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/IcomTech";
#else
    // Unix
    return pathRet / ".icomtech";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty icomtech.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override icomtech.conf
